...
void CRenderTarget::u_setrt(u32 W, u32 H, ID3DRenderTargetView* _1, ID3DRenderTargetView* _2,
                            ID3DRenderTargetView* _3, ID3DDepthStencilView* zb) {
    // VERIFY									(_1);
    dwWidth = W;
    dwHeight = H;
    // VERIFY									(_1);
...
CRenderTarget::CRenderTarget() {
    ...
    if (RImplementation.o.dx10_msaa) {
        ...
        for (int i = 0; i < bound; ++i) {
            ...
            static_cast<CBlender_accum_direct_mask_msaa*>(b_accum_mask_msaa[i])
            ...
            static_cast<CBlender_accum_direct_msaa*>(b_accum_direct_msaa[i])
                ->SetDefine("ISAMPLE", SampleDefs[i]);
            ...
            static_cast<CBlender_accum_point_msaa*>(b_accum_point_msaa[i])
                ->SetDefine("ISAMPLE", SampleDefs[i]);
            static_cast<CBlender_accum_reflected_msaa*>(b_accum_reflected_msaa[i])
                ->SetDefine("ISAMPLE", SampleDefs[i]);
    ...
    if (RImplementation.o.HW_smap) {
        ...
        if (RImplementation.o.advancedpp) {
            ...
            if (RImplementation.o.dx10_msaa) {
                ...
                for (int i = 0; i < bound; ++i) {
                    ...
                    // (b_accum_direct_volumetric_sun_msaa[i],			"r3\\accum_direct");
        ...
        // size,size,D3DFMT_R32F);  rt_smap_depth				= NULL;  R_CHK
        ...
        // (b_accum_mask,				"r2\\accum_mask");  s_accum_direct.create
        // (b_accum_direct,			"r2\\accum_direct");  if
    ...
    //	Or make DX10 rain switch dynamic?
    {
        ...
        if (RImplementation.o.dx10_msaa) {
            ...
            for (int i = 0; i < bound; ++i) {
                ...
                s_accum_spot_msaa[i].create(b_accum_spot_msaa[i], "r2\\accum_spot_s",
    ...
    // TONEMAP
    {
        ...
        for (u32 it = 0; it < HW.Caps.iGPUNum * 2; it++) {
            ...
            // D3DCLEAR_TARGET,	0x7f7f7f7f,	1.0f, 0L));
            FLOAT ColorRGBA[4] = { 127.0f / 255.0f, 127.0f / 255.0f, 127.0f / 255.0f,
                                   127.0f / 255.0f };
    ...
    //SampleCount); 	s_ssao.create				(b_ssao, "r2\\ssao");
    ...
    // COMBINE
    {
        static D3DVERTEXELEMENT9 dwDecl[] = { { 0, 0, D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT,
    ...
    // Build textures
    {
        ...
        if (1) {
            ...
            //	t_noise[it]					= dxRenderDeviceRender::Instance().Resources->_CreateTexture
            ...
            for (u32 y = 0; y < TEX_jitter; y++) {
                for (u32 x = 0; x < TEX_jitter; x++) {
                    ...
                    for (u32 it = 0; it < TEX_jitter_count - 1; it++) {
                        u32* p = (u32*)(LPBYTE(subData[it].pSysMem) + y * subData[it].SysMemPitch +
                                        x * 4);
            ...
            for (int it = 0; it < TEX_jitter_count - 1; it++) {
                ...
                // R_CHK						(t_noise_surf[it]->LockRect
                // (0,&R[it],0,0));
            }
            ...
            //	Create noise mipped
            {
                ...
                t_noise_mipped =
                    dxRenderDeviceRender::Instance().Resources->_CreateTexture(r2_jitter_mipped);
                t_noise_mipped->surface_set(t_noise_surf_mipped);

                //	Update texture. Generate mips.

                HW.pContext->CopySubresourceRegion(t_noise_surf_mipped, 0, 0, 0, 0, t_noise_surf[0],
                                                   0, 0);
