...
void CMapLocation::LoadSpot(LPCSTR type, bool bReload) {
    ...
    if (node) {
        ...
        m_spot_border_names[0] =
            g_uiSpotXml->ReadAttrib(path, 0, "spot_a", "level_map_spot_border");
    ...
    if (node) {
        ...
        m_spot_border_names[4] =
            g_uiSpotXml->ReadAttrib(path, 0, "spot_a", "complex_map_spot_border");
...
void CMapLocation::UpdateSpot(CUICustomMap* map, CMapSpot* sp) {
    if (map->MapName() == GetLevelName()) {
        ...
        m_position_on_map = map->ConvertRealToLocal(
            position, (map->Heading()) ? false : true); // for visibility calculating
        ...
        bool res =
            ai().graph_engine().search(ai().game_graph(), Actor()->ai_location().game_vertex_id(),
...
CRelationMapLocation::CRelationMapLocation(const shared_str& type, u16 object_id,
    ...
    if (m_last_relation == ALife::eRelationTypeEnemy ||
        ...
        if (_object_) {
            ...
            if (ea && !ea->g_Alive())
				...
				{
					if (pAct->Position().distance_to(pObj->Position())<
						bkp->m_fShowNearestEnemiesDistance)
						vis_res = true;
					else
						vis_res = Actor()->memory().visual().visible_now(pObj);
				}
