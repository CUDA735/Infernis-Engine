...
void CInventory::Take(CGameObject* pObj, bool bNotActivate, bool strict_placement) {
    ...
    // for unknown reason net_Import arrived for object that has a parent, so correction prediction
    // schema will crash
...
bool CInventory::Slot(u16 slot_id, PIItem pIItem, bool bNotActivate, bool strict_placement) {
    ...
    if (!IsGameTypeSingle()) {
        ...
        if (GetOwner()->object_id() != real_parent) {
            Msg("! WARNING: CL: actor [%d] tries to place to slot not own item [%d], that has "
                "parent [%d]",
    ...
    if (!strict_placement && !CanPutInSlot(pIItem, slot_id)) {
        ...
        Msg("there is item %s[%d,%x] in slot %d[%d,%x]",
            ...
            ItemFromSlot(pIItem->CurrSlot())->object().ID(), ItemFromSlot(pIItem->CurrSlot()),
            pIItem->CurrSlot(), pIItem->object().ID(), pIItem);
#endif
    ...
    if (!IsGameTypeSingle()) {
        if (it_ruck != m_ruck.end()) {
            ...
            u16 real_parent =
                pIItem->object().H_Parent() ? pIItem->object().H_Parent()->ID() : u16(-1);
            R_ASSERT2(
...
bool CInventory::Ruck(PIItem pIItem, bool strict_placement) {
    ...
    if (!IsGameTypeSingle()) {
        u16 real_parent = pIItem->object().H_Parent() ? pIItem->object().H_Parent()->ID() : u16(-1);
        if (GetOwner()->object_id() != real_parent) {
            Msg("! WARNING: CL: actor [%d] tries to place to ruck not own item [%d], that has "
                "parent [%d]",
                GetOwner()->object_id(), pIItem->object_id(), real_parent);
            return false;
        }
    ...
    if (in_slot) {
        ...
        if (!IsGameTypeSingle()) {
            ...
            R_ASSERT2(item_parent_id == inventory_owner_id,
                      make_string("! ERROR: CL: Actor[%d] tries to place to ruck not own item "
                                  "[%d], real item owner is [%d]",
...
void CInventory::Activate(u16 slot, bool bForce) {
    ...
    if (GetActiveSlot() == slot || (GetNextActiveSlot() == slot && !bForce)) {
        ...
        //				LogStackTrace	("");
        //			Msg				( "[%6d][%s] CInventory::Activate changing
        //next active slot to %d", Device.dwTimeGlobal, name, slot );
        //		}
        m_iNextActiveSlot = slot;
#ifdef DEBUG
//		Msg("--- There's no need to activate slot [%d], next active slot is [%d]", slot,
//m_iNextActiveSlot);
#endif
        return;
    }
...
#ifdef DEBUG
//	Msg("--- Activating slot [%d], inventory owner: [%s], Frame[%d]", slot, m_pOwner->Name(),
//Device.dwFrame);
#endif // #ifdef DEBUG
    ...
    if (GetActiveSlot() == NO_ACTIVE_SLOT) {
        if (tmp_item) {
            ...
            //!xr_strcmp("jup_b43_stalker_assistant_pri6695", name) )
            //					LogStackTrace	("");
            //				Msg				( "[%6d][%s] CInventory::Activate changing next active slot2 to
            //%d", Device.dwTimeGlobal, name, slot );
            //			}
            m_iNextActiveSlot = slot;
        } else {
...
#ifdef DEBUG
//			Msg("--- Inventory owner [%s]: send deactivate item [%s]", m_pOwner->Name(),
//active_item->NameItem());
#endif         // #ifdef DEBUG
        ...
        //				LogStackTrace	("");
        //			Msg				( "[%6d][%s] CInventory::Activate changing
        //next active slot3 to %d", Device.dwTimeGlobal, name, slot );
        //		}
        m_iNextActiveSlot = slot;
    }
}
...
void CInventory::Update() {
    if (OnServer()) {
        if (m_iActiveSlot != m_iNextActiveSlot) {
            ...
            //!xr_strcmp("jup_b43_stalker_assistant_pri6695", name) )
            //					LogStackTrace	("");
            //				Msg					("[%6d][%s] CInventory::Activate changing active slot from %d to
            //next active slot %d", Device.dwTimeGlobal, name, m_iActiveSlot, GetNextActiveSlot() );
            //			}
            m_iActiveSlot = GetNextActiveSlot();
        }
        if ((GetNextActiveSlot() != NO_ACTIVE_SLOT) && ActiveItem() &&
...
#ifndef MASTER_GOLD
            Msg("Set slots blocked: activating prev slot [%d], Frame[%d]", PrevActiveSlot,
                Device.dwFrame);
...
void CInventory::TryDeactivateActiveSlot() {
    ...
    PIItem next_active_item =
        (NextActiveSlot != NO_ACTIVE_SLOT) ? ItemFromSlot(NextActiveSlot) : NULL;
