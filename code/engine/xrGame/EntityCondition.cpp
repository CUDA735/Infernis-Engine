...
void CEntityCondition::LoadCondition(LPCSTR entity_section) {
    LPCSTR section =
        READ_IF_EXISTS(pSettings, r_string, entity_section, "condition_sect", entity_section);
    ...
    m_use_limping_state =
        !!(READ_IF_EXISTS(pSettings, r_bool, section, "use_limping_state", FALSE));
    ...
    m_fInvulnerableTimeDelta =
        READ_IF_EXISTS(pSettings, r_float, section, "invulnerable_time", 0.0f) / 1000.f;
}
...
void CEntityCondition::UpdateWounds() {
    ...
    m_WoundVector.erase(
        std::remove_if(m_WoundVector.begin(), m_WoundVector.end(), &RemoveWoundPred),
        m_WoundVector.end());
}
...
CWound* CEntityCondition::ConditionHit(SHit* pHDS) {
    ...
    hit_power = HitOutfitEffect(hit_power_org, pHDS->hit_type, pHDS->boneID, pHDS->armor_piercing,
                                bAddWound);
    ...
    case ALife::eHitTypeWound:
        ...
        m_fDeltaPower -= hit_power * m_fPowerHitPart;
        break;
    default: { R_ASSERT2(0, "unknown hit type"); } break;
    }
...
void CEntityCondition::UpdateRadiation() {
    if (m_fRadiation > 0) {
        ...
        m_fDeltaHealth -=
            CanBeHarmed() ? m_change_v.m_fV_RadiationHealth * m_fRadiation * m_fDeltaTime : 0.0f;
    }
}
