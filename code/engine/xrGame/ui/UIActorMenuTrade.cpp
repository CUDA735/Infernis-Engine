...
bool CUIActorMenu::ToActorTrade(CUICellItem* itm, bool b_use_cursor_pos) {
    ...
    //	if(m_pActorInvOwner->inventory().CanPutInRuck(iitem))
    {
        ...
        bool result =
            (old_owner_type != iActorBag) ? m_pActorInvOwner->inventory().Ruck(iitem) : true;
        VERIFY(result);
...
bool CUIActorMenu::CanMoveToPartner(PIItem pItem) {
    ...
    if (!m_pPartnerInvOwner->trade_parameters().enabled(CTradeParameters::action_buy(0),
...
void CUIActorMenu::OnBtnPerformTradeBuy(CUIWindow* w, void* d) {
    ...
    if ((actor_money >= 0) /*&& ( partner_money >= 0 )*/ &&
        ...
        //		TransferItems( m_pTradeActorList,   m_pTradePartnerBagList, m_partner_trade,
        //true );
...
void CUIActorMenu::TransferItems(CUIDragDropListEx* pSellList, CUIDragDropListEx* pBuyList,
                                 CTrade* pTrade, bool bBuying) {
