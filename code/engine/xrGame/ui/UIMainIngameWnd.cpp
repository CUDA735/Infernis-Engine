...
void CUIMainIngameWnd::Init() {
    ...
    //	AttachChild					(&UIStaticHealth);	xml_init.InitStatic			(uiXml, "static_health",
    //0, &UIStaticHealth); 	AttachChild					(&UIStaticArmor);
...
void CUIMainIngameWnd::Draw() {
    ...
    if ((UIStaticDiskIO_start_time + 1.0f) < Device.fTimeGlobal)
        ...
        u32 alpha = clampr(
            iFloor(255.f * (1.f - (Device.fTimeGlobal - UIStaticDiskIO_start_time) / 1.f)), 0, 255);
    ...
    if (!IsGameTypeSingle()) {
        float luminocity =
            smart_cast<CGameObject*>(Level().CurrentEntity())->ROS()->get_luminocity();
...
void CUIMainIngameWnd::UpdateMainIndicators() {
    ...
    float satiety_koef = (satiety - satiety_critical) /
                         (satiety >= satiety_critical ? 1 - satiety_critical : satiety_critical);
    if (satiety_koef > 0.5)
    ...
    CCustomOutfit* outfit =
        smart_cast<CCustomOutfit*>(pActor->inventory().ItemFromSlot(OUTFIT_SLOT));
