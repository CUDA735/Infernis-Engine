...
bool RemoveItemFromList(CUIDragDropListEx* lst, PIItem pItem) { // fixme
    CUICellItem* ci = NULL;
...
void CUIActorMenu::OnInventoryAction(PIItem pItem, u16 action_type) {
    CUIDragDropListEx* all_lists[] = { m_pInventoryBeltList,      m_pInventoryPistolList,
									   ...
									   m_pInventoryKnifeList,     m_pInventoryBinocularList,
                                       m_pInventoryBackpackList,  m_pInventoryBagList,
...
bool CUIActorMenu::ToBelt(CUICellItem* itm, bool b_use_cursor_pos) {
    ...
    if (m_pActorInvOwner->inventory().CanPutInBelt(iitem)) {
        ...
        // ColorizeItem						(itm, false);
        return true;
    } else { // in case belt slot is busy
        ...
        //		VERIFY								(slot_cell &&
        //((PIItem)slot_cell->m_pData)==_iitem);
...
bool CUIActorMenu::OnItemDropped(PIItem itm, CUIDragDropListEx* new_owner,
...
void CUIActorMenu::PropertiesBoxForWeapon(CUICellItem* cell_item, PIItem item, bool& b_show) {
    ...
    if (pWeapon->GrenadeLauncherAttachable()) {
        if (pWeapon->IsGrenadeLauncherAttached()) {
            m_UIPropertiesBox->AddItem("st_detach_gl", NULL,
    ...
    if (smart_cast<CWeaponMagazined*>(pWeapon) && IsGameTypeSingle()) {
        ...
        if (!b) {
            for (u32 i = 0; i < cell_item->ChildsCount(); ++i) {
                CWeaponMagazined* weap_mag =
                    smart_cast<CWeaponMagazined*>((CWeapon*)cell_item->Child(i)->m_pData);
...
void CUIActorMenu::PropertiesBoxForAddon(PIItem item, bool& b_show) {
    ...
    if (pSilencer) {
        if (item_in_slot_2 && item_in_slot_2->CanAttach(pSilencer)) {
            ...
            str.printf("%s %s", str.c_str(), item_in_slot_2->m_name.c_str());
            m_UIPropertiesBox->AddItem(str.c_str(), (void*)item_in_slot_2, INVENTORY_ATTACH_ADDON);
            //			m_UIPropertiesBox->AddItem( "st_attach_silencer_to_pistol",
            //(void*)item_in_slot_2, INVENTORY_ATTACH_ADDON );
            b_show = true;
        }
        if (item_in_slot_3 && item_in_slot_3->CanAttach(pSilencer)) {
            ...
            str.printf("%s %s", str.c_str(), item_in_slot_3->m_name.c_str());
            m_UIPropertiesBox->AddItem(str.c_str(), (void*)item_in_slot_3, INVENTORY_ATTACH_ADDON);
            //			m_UIPropertiesBox->AddItem( "st_attach_silencer_to_rifle",
            //(void*)item_in_slot_3, INVENTORY_ATTACH_ADDON );
            b_show = true;
        }
        return;
    }
...
void CUIActorMenu::ProcessPropertiesBoxClicked(CUIWindow* w, void* d) {
    ...
    case INVENTORY_ATTACH_ADDON: {
        PIItem item =
            CurrentIItem(); // temporary storing because of AttachAddon is setting curiitem to NULL
    ...
    case INVENTORY_UNLOAD_MAGAZINE: {
        ...
        for (u32 i = 0; i < cell_item->ChildsCount(); ++i) {
            ...
            CWeaponMagazined* child_weap_mag =
                smart_cast<CWeaponMagazined*>((CWeapon*)child_itm->m_pData);
            if (child_weap_mag) {
