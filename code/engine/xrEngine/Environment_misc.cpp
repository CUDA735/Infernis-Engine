...
CEnvAmbient::SEffect* CEnvAmbient::create_effect(CInifile& config, LPCSTR id) {
    ...
    if (config.line_exist(id, "wind_blast_strength")) {
        ...
        result->wind_blast_direction.setHP(deg2rad(config.r_float(id, "wind_blast_longitude")),
                                           0.f);
...
void CEnvDescriptor::load(CEnvironment& environment, CInifile& config) {
    ...
    R_ASSERT3((tm.x >= 0) && (tm.x < 24) && (tm.y >= 0) && (tm.y < 60) && (tm.z >= 0) &&
                  (tm.z < 60),
...
void CEnvDescriptorMixer::lerp(CEnvironment*, CEnvDescriptor& A, CEnvDescriptor& B, float f,
                               CEnvModifier& Mdf, float modifier_power) {
    ...
    */
    ...
    //Mdf.far_plane)*psVisDistance*modif_power;
    if (Mdf.use_flags.test(eViewDist))
        far_plane =
            (fi * A.far_plane + f * B.far_plane + Mdf.far_plane) * psVisDistance * modif_power;
    else
        far_plane = (fi * A.far_plane + f * B.far_plane) * psVisDistance;

    //.	fog_color.lerp
    //(A.fog_color,B.fog_color,f).add(Mdf.fog_color).mul(modif_power);
    ...
    // colors
    //.	sky_color.lerp
    //(A.sky_color,B.sky_color,f).add(Mdf.sky_color).mul(modif_power);
