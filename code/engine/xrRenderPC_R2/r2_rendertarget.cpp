...
void CRenderTarget::u_setrt(const ref_rt& _1, const ref_rt& _2, const ref_rt& _3,
...
void CRenderTarget::u_setrt(u32 W, u32 H, IDirect3DSurface9* _1, IDirect3DSurface9* _2,
                            IDirect3DSurface9* _3, IDirect3DSurface9* zb) {
    VERIFY(_1);
    dwWidth = W;
    dwHeight = H;
    VERIFY(_1);
...
void generate_jitter(DWORD* dest, u32 elem_count) {
    ...
    for (u32 it = 0; it < elem_count; it++, dest++)
        *dest = color_rgba(samples[2 * it].x, samples[2 * it].y, samples[2 * it + 1].y,
                           samples[2 * it + 1].x);
}
...
CRenderTarget::CRenderTarget() {
    ...
    if (RImplementation.o.HW_smap) {
        ...
        rt_smap_depth = NULL;
        R_CHK(HW.pDevice->CreateDepthStencilSurface(size, size, D3DFMT_D24X8, D3DMULTISAMPLE_NONE,
                                                    0, TRUE, &rt_smap_ZB, NULL));
        s_accum_mask.create(b_accum_mask, "r2\\accum_mask");
        s_accum_direct.create(b_accum_direct, "r2\\accum_direct");
        s_accum_direct_cascade.create(b_accum_direct_cascade, "r2\\accum_direct_cascade");
        if (RImplementation.o.advancedpp) {
    ...
    // BLOOM
    {
        ...
        u32 fvf_build = D3DFVF_XYZRHW | D3DFVF_TEX4 | D3DFVF_TEXCOORDSIZE2(0) |
                        D3DFVF_TEXCOORDSIZE2(1) | D3DFVF_TEXCOORDSIZE2(2) | D3DFVF_TEXCOORDSIZE2(3);
        u32 fvf_filter =
            (u32)D3DFVF_XYZRHW | D3DFVF_TEX8 | D3DFVF_TEXCOORDSIZE4(0) | D3DFVF_TEXCOORDSIZE4(1) |
    ...
    // COMBINE
    {
        ...
        u32 fvf_aa_blur = D3DFVF_XYZRHW | D3DFVF_TEX4 | D3DFVF_TEXCOORDSIZE2(0) |
    ...
    // Build textures
    {
        ...
        {
            // Surface
            R_CHK(D3DXCreateVolumeTexture(HW.pDevice, TEX_material_LdotN, TEX_material_LdotH, 4, 1,
                                          0, D3DFMT_A8L8, D3DPOOL_MANAGED, &t_material_surf));
            ...
            for (u32 slice = 0; slice < 4; slice++) {
                for (u32 y = 0; y < TEX_material_LdotH; y++) {
                    for (u32 x = 0; x < TEX_material_LdotN; x++) {
                        u16* p =
                            (u16*)(LPBYTE(R.pBits) + slice * R.SlicePitch + y * R.RowPitch + x * 2);
        ...
        if (1) {
            ...
            for (int it1 = 0; it1 < TEX_jitter_count - 1; it1++) {
                ...
                R_CHK(D3DXCreateTexture(HW.pDevice, TEX_jitter, TEX_jitter, 1, 0, D3DFMT_Q8W8V8U8,
                                        D3DPOOL_MANAGED, &t_noise_surf[it1]));
            ...
            R_CHK(D3DXCreateTexture(HW.pDevice, TEX_jitter, TEX_jitter, 1, 0, D3DFMT_A32B32G32R32F,
                                    D3DPOOL_MANAGED, &t_noise_surf[it]));
            ...
            for (u32 y = 0; y < TEX_jitter; y++) {
                for (u32 x = 0; x < TEX_jitter; x++) {
                    ...
                    float* p =
                        (float*)(LPBYTE(R[it].pBits) + y * R[it].Pitch + x * 4 * sizeof(float));
    ...
    HW.pDevice->CreateOffscreenPlainSurface(Device.dwWidth, Device.dwHeight, desc.Format,
...
void CRenderTarget::reset_light_marker(bool bResetStencil) {
    ...
    if (bResetStencil) {
        ...
        RCache.set_Stencil(TRUE, D3DCMP_ALWAYS, dwLightMarkerID, 0x00, 0xFE, D3DSTENCILOP_ZERO,
                           D3DSTENCILOP_ZERO, D3DSTENCILOP_ZERO);
        // RCache.set_Stencil	(TRUE,D3DCMP_ALWAYS,dwLightMarkerID,0x00,0xFF, D3DSTENCILOP_ZERO,
        // D3DSTENCILOP_ZERO, D3DSTENCILOP_ZERO);
        RCache.set_Element(s_occq->E[1]);
        RCache.set_Geometry(g_combine);
        RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
        ...
        /*
           ...
           = 0.5f; float	_dh					= 0.5f; FVF::TL* pv
           = (FVF::TL*) RCache.Vertex.Lock	(4,g_combine->vb_stride,Offset); pv->set
           (-_dw,		_h-_dh,		eps,	1.f, C, 0, 0);	pv++; pv->set
           (-_dw,		-_dh,		eps,	1.f, C, 0, 0);	pv++; pv->set
           (_w-_dw,	_h-_dh,		eps,	1.f, C, 0, 0);	pv++;
                        pv->set						(_w-_dw,	-_dh,
           eps,	1.f, C, 0, 0);	pv++; RCache.Vertex.Unlock		(4,g_combine->vb_stride);
