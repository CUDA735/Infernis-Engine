...
void CRender::create() {
    ...
    */
    ...
    o.fp16_blend =
        HW.support(D3DFMT_A16B16G16R16F, D3DRTYPE_TEXTURE, D3DUSAGE_QUERY_POSTPIXELSHADER_BLENDING);
    ...
    if ((HW.Caps.id_vendor == 0x10DE) && (HW.Caps.id_device >= 0x40)) {
        // o.nvstencil = HW.support	((D3DFORMAT)MAKEFOURCC('R','A','W','Z'), D3DRTYPE_SURFACE,
        // 0);  o.nvstencil = TRUE;
        o.nvstencil = (S_OK == HW.pD3D->CheckDeviceFormat(
                                   D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, 0,
                                   D3DRTYPE_TEXTURE, (D3DFORMAT MAKEFOURCC('R', 'A', 'W', 'Z'))));
    }
...
HRESULT CRender::shader_compile(LPCSTR name, DWORD const* pSrcData, UINT SrcDataLen,
    ...
    if (FAILED(_result)) {
        ...
        _result = D3DXCompileShader((LPCSTR)pSrcData, SrcDataLen, defines, pInclude, pFunctionName,
                                    pTarget, Flags | D3DXSHADER_USE_LEGACY_D3DX9_31_DLL,
