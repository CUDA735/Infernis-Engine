...
#include "xrRenderCommon/dxEnvironmentRender.h"
...
void CRenderTarget::phase_combine() {
    ...
    if (!_menu_pp) {
        ...
        Fvector4 ambclr = { std::max(envdesc.ambient.x * 2, minamb),
                            std::max(envdesc.ambient.y * 2, minamb),
                            std::max(envdesc.ambient.z * 2, minamb), 0 };
        ambclr.mul(ps_r2_sun_lumscale_amb);

        //.		Fvector4	envclr			= { envdesc.sky_color.x*2+EPS,
        //envdesc.sky_color.y*2+EPS,	envdesc.sky_color.z*2+EPS,	envdesc.weight
        //};
        ...
        // pv->set						(hclip(EPS,		_w),	hclip(_h+EPS,	_h),
        // p0.x, p1.y);	pv++;
        // pv->set						(hclip(EPS,		_w),	hclip(EPS,
        // _h),	p0.x, p0.y);	pv++;
        // pv->set						(hclip(_w+EPS,	_w),	hclip(_h+EPS,	_h),
        // p1.x, p1.y);	pv++;
        // pv->set						(hclip(_w+EPS,	_w),	hclip(EPS,		_h),
        // p1.x, p0.y);	pv++;  RCache.Vertex.Unlock		(4,g_combine_VP->vb_stride);
        ...
        IDirect3DBaseTexture9* e0 =
            _menu_pp ? 0 : envdescren.sky_r_textures_env[0].second->surface_get();
        IDirect3DBaseTexture9* e1 =
            _menu_pp ? 0 : envdescren.sky_r_textures_env[1].second->surface_get();
    ...
    BOOL bDistort = RImplementation.o.distortion_enabled; // This can be modified
    {
        ...
        if (bDistort) {
            ...
            RCache.set_Stencil(FALSE);
            RCache.set_ColorWriteEnable();
            CHK_DX(HW.pDevice->Clear(0L, NULL, D3DCLEAR_TARGET, color_rgba(127, 127, 0, 127), 1.0f,
                                     0L));
    ...
    if (1) {
        ...
        //.		RCache.set_c				("dof_params",	ps_r2_dof.x,
        //ps_r2_dof.y, ps_r2_dof.z, ps_r2_dof_sky);
    ...
    //	if FP16-BLEND !not! supported - draw flares here, overwise they are already in the bloom
    //target
    /* if (!RImplementation.o.fp16_blend)*/ g_pGamePersistent->Environment()
...
(4,g_combine->vb_stride,Offset);
                pv->set						((IX+0)*_w+EPS,
(IY+1)*_h+EPS,	EPS,	1.f, C, p0.x, p1.y);	pv++;
                pv->set						((IX+0)*_w+EPS,
(IY+0)*_h+EPS,	EPS,	1.f, C, p0.x, p0.y);	pv++;
                pv->set						((IX+1)*_w+EPS,
(IY+1)*_h+EPS,	EPS,	1.f, C, p1.x, p1.y);	pv++;
                pv->set						((IX+1)*_w+EPS,
(IY+0)*_h+EPS,	EPS,	1.f, C, p1.x, p0.y);	pv++; RCache.Vertex.Unlock
(4,g_combine->vb_stride);
...
void CRenderTarget::phase_wallmarks() {
    ...
    RCache.set_CullMode(CULL_CCW);
    RCache.set_ColorWriteEnable(D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN |
